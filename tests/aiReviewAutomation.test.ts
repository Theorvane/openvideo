...
describe("AI pull request automation", () => {
  it("requests the AI reviewer without checking out untrusted code", () => {
  ...
  it("only enables squash auto-merge after the AI approves the current HEAD", () => {
    ...
    expect(workflow).toContain("labels/ai-review-requested")
    expect(workflow).toContain('github.event.pull_request.base.ref }}" = "main"')
    expect(workflow).toContain('gh pr merge "$PR_NUMBER" --auto --merge --delete-branch')
    expect(workflow).toContain('gh pr merge "$PR_NUMBER" --auto --squash --delete-branch')
  })
})
