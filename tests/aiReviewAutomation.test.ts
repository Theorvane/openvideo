...
describe("AI pull request automation", () => {
  ...
  it("enables squash auto-merge only for current-HEAD AI approvals", () => {
    ...
    expect(workflow).not.toContain("github.event.review")
    expect(workflow).toContain('.state == "APPROVED"')
    expect(workflow).toContain('.author.login == "sjungwon03-ai"')
    expect(workflow).toContain('.commit.oid == .headRefOid')
    ...
    expect(workflow).toContain('gh pr merge "$number" --repo "$GITHUB_REPOSITORY" --auto --squash --delete-branch')
    expect(workflow).not.toContain("actions/checkout")
  })
})
